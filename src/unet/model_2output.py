...
def build_unet(
    ...
    # Decoder (3x Up: *2, *4, *8 -> zurück auf 600)
    ...
    main_out = L.Conv2D(
        ...
        1,
        activation="softmax",
        name="main",          # Muss mit Dataset-Label-Key übereinstimmen
    )(features)

    if add_row_head:
        # Head 2: Binäre Dimerreihen-Maske 0/1
        rows_out = L.Conv2D(
            1,
            1,
            activation="sigmoid",   # Für BinaryCrossentropy(from_logits=False)
            name="rows",            # Muss mit Dataset-Label-Key übereinstimmen
        )(features)

        return tf.keras.Model(
            inputs=inputs,
            outputs={"main": main_out, "rows": rows_out},
            name="unet_multitask_rows",
        )
    else:
        # Fallback: altes Single-Head-Modell
        return tf.keras.Model(
            inputs=inputs,
            outputs=main_out,
            name="unet_multiclass_3levels",
        )
